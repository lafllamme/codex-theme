...
const SHARED_DIFF_FILES: FileChangeItem[] = [
  ...
  },
  {
    ...
    removed: 18,
    lines: [
      { kind: 'context', left: 96, right: 96, text: 'const statusCounts = { unstaged: 1, staged: 0 } as const' },
      { kind: 'context', left: 97, right: 97, text: 'const showBulkActions = computed(() => {' },
      { kind: 'context', left: 98, right: 98, text: '  if (diffSections.value.length <= 1) return false' },
      { kind: 'context', left: 99, right: 99, text: '  return diffSections.value.some(section => !diffStore.isSectionCollapsed(section.id))' },
      { kind: 'context', left: 100, right: 100, text: '})' },
      { kind: 'context', left: 102, right: 102, text: '<ComposerDropdownMenu :open="openMenuKey === \'status\'" ...>' },
      { kind: 'context', left: 108, right: 108, text: '<template #trigger="{ toggle }">' },
      { kind: 'context', left: 109, right: 109, text: '<button class="h-7 inline-flex ...">' },
      { kind: 'context', left: 110, right: 110, text: '<span class="truncate">{{ selectedStatusLabel }}</span>' },
      { kind: 'added', left: '', right: 111, text: '<span class="...">Unstaged</span>' },
      { kind: 'added', left: '', right: 112, text: '<span class="... rounded-full ...">1</span>' },
      { kind: 'context', left: 113, right: 113, text: '<Icon name="ph:caret-down-bold" ... />' },
      { kind: 'context', left: 114, right: 114, text: '</button>' },
      { kind: 'context', left: 115, right: 115, text: '</template>' },
      { kind: 'context', left: 116, right: 116, text: '<div class="grid gap-0.5 text-[13px]">' },
      ...
      { kind: 'context', left: 149, right: 149, text: '</button>' },
      { kind: 'context', left: 150, right: 150, text: '</div>' },
      { kind: 'context', left: 151, right: 151, text: '</ComposerDropdownMenu>' },
      { kind: 'context', left: 152, right: 152, text: '<div class="inline-flex items-center gap-1 ...">' },
      { kind: 'context', left: 153, right: 153, text: '<button class="h-6 w-6 inline-flex ...">' },
      { kind: 'context', left: 154, right: 154, text: '<Icon name="ph:dots-three" ... />' },
      ...
      { kind: 'context', left: 177, right: 177, text: '}' },
      { kind: 'context', left: 178, right: 178, text: '' },
      { kind: 'context', left: 179, right: 179, text: '@media (max-width: 1180px) {' },
      { kind: 'context', left: 180, right: 180, text: '  .diff-drawer { position: fixed; inset: 104px 8px 8px; }' },
      { kind: 'context', left: 181, right: 181, text: '  .diff-drawer--open { transform: translateX(0); }' },
      { kind: 'context', left: 182, right: 182, text: '}' },
      { kind: 'context', left: 212, right: 212, text: '.diff-drawer--open { opacity: 1; transform: translateX(0); }' },
      { kind: 'context', left: 213, right: 213, text: '.diff-scroll { scrollbar-width: thin; }' },
      { kind: 'context', left: 214, right: 214, text: '.diff-scroll::-webkit-scrollbar { height: 10px; width: 10px; }' },
    ],
  },
]
