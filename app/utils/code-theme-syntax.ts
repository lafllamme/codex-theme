...
const BASE_TOKEN_SET: Record<OfficialCodeThemeId, SyntaxTokenSet> = {
...
const TOKEN_WEIGHT: Record<keyof SyntaxTokenSet, number> = {
  default: 2.2,
  keyword: 1.3,
  string: 1.2,
  comment: 0.8,
...
function ensureReadableToken(token: string, surface: string, ink: string, minRatio: number): string {
...
function scoreTokenSet(payload: CodexThemePayload, tokens: SyntaxTokenSet): number {
  const surface = payload.theme.surface
  const accent = payload.theme.accent
  const { diffAdded, diffRemoved, skill } = payload.theme.semanticColors
  const surfaceLum = luminance(surface)

  let score = 0

  ;(Object.keys(TOKEN_MIN_RATIO) as Array<keyof SyntaxTokenSet>).forEach((key) => {
  ...
  const semanticTokens: Array<keyof SyntaxTokenSet> = ['keyword', 'string', 'function', 'type', 'number']
  const avgTokenLum = semanticTokens
    .map(key => luminance(tokens[key]))
    .reduce((a, b) => a + b, 0) / semanticTokens.length
  ...
  const accentDistanceAvg = ['keyword', 'function', 'type']
    .map(key => colorDistance(accent, tokens[key as keyof SyntaxTokenSet]))
    .reduce((a, b) => a + b, 0) / 3

  // Avoid monochrome-feeling code when UI accent is dominant.
  if (colorDistance(accent, tokens.keyword) < 38)
    score -= 1.4

  // Keep token palette related to the actual UI semantic colors.
...
function normalizedTokens(payload: CodexThemePayload, themeId: OfficialCodeThemeId): SyntaxTokenSet {
  const base = BASE_TOKEN_SET[themeId] ?? BASE_TOKEN_SET[DEFAULT_CODE_THEME_ID]
  const surface = payload.theme.surface
  const ink = payload.theme.ink
...
export function recommendCodeThemeId(payload: CodexThemePayload): OfficialCodeThemeId {
  let bestThemeId: OfficialCodeThemeId = DEFAULT_CODE_THEME_ID
