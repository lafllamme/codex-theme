...
const props = withDefaults(defineProps<TerminalBackgroundProps>(), {
  scale: 1.5,
  gridMul: () => [2, 1],
  digitSize: 1.2,
  timeScale: 0.5,
  ...
  mouseReact: true,
  mouseStrength: 0.5,
  dpr: () => (import.meta.client ? Math.min(window.devicePixelRatio || 1, 2) : 1),
...
function setup() {
  ...
  const update = (timeMs: number) => {
    ...
    if (props.pageLoadAnimation && loadAnimationStartRef.value === 0)
      loadAnimationStartRef.value = timeMs

    if (!props.pause) {
      const elapsed = (timeMs * 0.001 + timeOffsetRef.value) * props.timeScale
    ...
    if (props.pageLoadAnimation && loadAnimationStartRef.value > 0) {
      ...
      const animationElapsed = timeMs - loadAnimationStartRef.value
